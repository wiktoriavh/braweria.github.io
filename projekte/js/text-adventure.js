...
const treesap = document.getElementById("treesap");
...
let tatzelwurmHasEgg = false; // Hat der Tatzelwurm ihr Ei bekommen?
let stueppPlayTime = false; // Hat man mit dem Stüpp Junges gespielt?
let undineSinging = false; // Hat man die Quest für Udine gemacht?
...
optionA.addEventListener("click", () => {
	...
	checkItems();
	// Moved tiered conditional statements to a switch statement.
	switch (appState.path) {
		case "beginning": // Ansprechen
		...
		case "helpDilladapp": // Auswahl zwischen [Sumpf], Teich und Bäume
		...
		case "swamp": // Im Sumpf [Mutter finden] oder Ei in Ruhe lassen
		...
		case "findMother": // Treffen Tatzelwurm, [Ansprechen] oder Ignorieren
		...
		case "undine": // wenn man mit ihr singt
		...
		case "trees": // wenn man mit ihr singt
...
optionC.addEventListener("click", () => {
	...
	switch (appState.path) {
		...
		case "speakDilladapp": // ignorieren
			text.innerHTML = "'Ich hoffe, du findest noch deine Zutaten!', sagte Marie und führte ihr Spaziergang mit Luna fort, bis sie wieder zuhause war und mit Luna gekuschelt hat.",
				optionA.innerHTML = "Erneut probieren?";
			appState.path = "end";
			optionC.classList.add("hide");
			appState.items.bag = false;
		...
		case "undine": // wenn man NICHT mit ihr singt
		...
		case "speakStuepp": // Auswahl zwischen Sumpf, Teich und [Bäume]
